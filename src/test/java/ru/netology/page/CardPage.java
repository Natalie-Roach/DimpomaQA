...
public class CardPage {
    ...
    private static SelenideElement successNote = $(byText("Операция одобрена Банком."));
    private static SelenideElement errorNote = $(byText("Ошибка! Банк отказал в проведении операции."));
    ...
    public static void payInDebitButton() {
        payButton.click();
    }

    public static void payInCreditButton() {
        payInCreditButton.click();
    }

    public static void successfulWay() {
        successNote.shouldBe(Condition.visible, Duration.ofSeconds(20));
    }

    public static void unSuccessfulWay() {
        errorNote.shouldBe(Condition.visible, Duration.ofSeconds(20));
    }
